class BaseWallet:
    exchange_rates = {"RUB": 1, "USD": 60, "EUR": 70}

    def __init__(self, name: str, amount: float, currency: str):
        self.name = name
        self.amount = amount
        self.exchange_rate = BaseWallet.exchange_rates[currency]

    def __str__(self):        
        if self.exchange_rate == 1:
            self.type = 'Rubble Wallet'
            return f"{self.type} {self.name} {self.amount}"
        elif self.exchange_rate == 60:
            self.type = 'Dollar Wallet'
            return f"{self.type} {self.name} {self.amount}"
        else:
            self.type = 'Euro Wallet'
            return f"{self.type} {self.name} {self.amount}"
    ...
    def __add__(self, other):
        if isinstance(other, (int, float)):
            ...
            amount = other.amount * other.exchange_rate / self.exchange_rate
            return self.__class__(self.name, self.amount + amount)
    ...
    def __rsub__(self, other):
        return (self.__class__(self.name, 0) - self).__add__(other)
...
class RubbleWallet(BaseWallet):
    def __init__(self, name: str, amount: float):
        super().__init__(name, amount, "RUB")


class DollarWallet(BaseWallet):
    def __init__(self, name: str, amount: float):
        super().__init__(name, amount, "USD")


class EuroWallet(BaseWallet):
    def __init__(self, name: str, amount: float):
        super().__init__(name, amount, "EUR")
